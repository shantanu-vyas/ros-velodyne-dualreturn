...
namespace velodyne_rawdata
{
                             ...
                             VPointCloud &pc)
  {
	...
	/*have to interpolate for the second sequence in the nth block
	  do all of that then
	  block=0, block+=2 get all even blocks add to list
	  block=1, block+=2 get all odd blocks add to list
	 */
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle 
            ...
            if (pointInRange(distance)) {
              ...
              point.z = z_coord;
              point.intensity = (uint8_t) intensity;
			  //if block even push to one list if odd push to another make one pointcloud for even d odd
              pc.points.push_back(point);
              ++pc.width;
            }
          }
   ...
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vlp16_dual(const velodyne_msgs::VelodynePacket &pkt,
								  VPointCloud &pclast,VPointCloud &pcstrongest)
  {
    float azimuth;
    float azimuth_diff;
    float last_azimuth_diff=0;
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle 
            ...
            if (pointInRange(distance)) {
              ...
              point.intensity = (uint8_t) intensity;
			  //if block even push to one list if odd push to another make one pointcloud for even d odd
			  pclast.push_back(point);
			  if (block % 2 == 0)
			  	{
			  	  pclast.points.push_back(point);
				  pclast.width++;
				  //			  	  std::cout << "last" << std::endl;
			  	}
			  else
			  	{
			  	  pcstrongest.points.push_back(point);
				  pcstrongest.width++;
